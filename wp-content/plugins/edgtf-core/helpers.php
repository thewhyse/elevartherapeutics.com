...
if(!function_exists('edgt_core_portfolio_ajax_load_more')){
	...
	if($query_results->have_posts()):			
		...
		endwhile;
			...
			$html .= '<p>'. esc_html__('Sorry, no posts matched your criteria.', 'edgtf-core') .'</p>';
		endif;
...
if ( class_exists( 'WP_Block_Type' ) && edgt_core_theme_installed() ) {
	add_action( 'admin_head', 'conall_edge_meta_box_add' );
} else {
	add_action( 'add_meta_boxes', 'conall_edge_meta_box_add' );
}

if ( ! function_exists( 'conall_edge_create_meta_box_handler' ) ) {
	function conall_edge_create_meta_box_handler( $box, $key, $screen ) {
		add_meta_box(
			'edgtf-meta-box-'.$key,
			$box->title,

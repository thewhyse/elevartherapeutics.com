<?php
namespace ConallEdgeNamespace\Modules\Shortcodes\Icon;

use ConallEdgeNamespace\Modules\Shortcodes\ShortcodeInterface;

/**
 * Class Icon
 * @package ConallEdgeNamespace\Modules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
    ...
    public function render($atts, $content = null) {
        ...
        $html = conall_edge_get_shortcode_module_template_part('templates/icon', 'icon', '', $params);
        ...
        if(($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
           ($params['type'] == 'normal')
            ...
            if(!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:'.conall_edge_filter_px($params['custom_size']).'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type']) && $params['type'] !== 'normal') {
            ...
            if(!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: '.conall_edge_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'height: '.conall_edge_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'line-height: '.conall_edge_filter_px($shapeSize).'px';
            }
            ...
            if(!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
				...
				$iconHolderStyles[] = 'border-width: '.conall_edge_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.conall_edge_filter_px($params['border_radius']).'px';
                }
            }
        }

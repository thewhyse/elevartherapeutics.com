<?php
namespace ConallEdgeNamespace\Modules\Shortcodes\SocialShare;

use ConallEdgeNamespace\Modules\Shortcodes\ShortcodeInterface;
...
class SocialShare implements ShortcodeInterface {
	...
	public function render($atts, $content = null) {
		...
		$params['enable_social_share'] = (conall_edge_options()->getOptionValue('enable_social_share') == 'yes') ? true : false;

		//Is social share enabled for post type
		$post_type = get_post_type();
		$params['enabled'] = (conall_edge_options()->getOptionValue('enable_social_share_on_'.$post_type)) ? true : false;
		...
		if ($params['enable_social_share']) {
			if ($params['enabled']) {
				$html .= conall_edge_get_shortcode_module_template_part('templates/' . $params['type'], 'socialshare', '', $params);
			}
		}
	...
	private function getSocialNetworksParams($params) {
		...
		foreach ($this->socialNetworks as $net) {
			...
			$html = '';
			if (conall_edge_options()->getOptionValue('enable_'.$net.'_share') == 'yes') {
				...
				$params['custom_icon'] = (conall_edge_options()->getOptionValue($net.'_icon')) ? conall_edge_options()->getOptionValue($net.'_icon') : '';
				$html = conall_edge_get_shortcode_module_template_part('templates/parts/network', 'socialshare', '', $params);
	...
	private function getSocialNetworkShareLink($net, $image) {
		...
		switch ($net) {
			...
			case 'twitter':
                ...
                $twitter_via = (conall_edge_options()->getOptionValue('twitter_via') !== '') ? esc_attr__( ' via ', 'masterds-core' ) . conall_edge_options()->getOptionValue('twitter_via') . ' ' : '';
                $link        = 'window.open(\'https://twitter.com/intent/tweet?text=' . urlencode( conall_edge_the_excerpt_max_charlength( $count_char ) . $twitter_via ) . ' ' . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
				 break;
			case 'linkedin':
	...
	private function getSocialNetworkIcon($net, $type) {
		...
		switch ($net) {
			case 'facebook':
				$icon = ( $type == 'circle' ) ? 'social_facebook_circle' : 'fa fa-facebook';
				break;
			case 'twitter':
				$icon = ( $type == 'circle' ) ? 'social_twitter_circle' : 'fa fa-twitter';
				break;
			case 'linkedin':
				$icon = ( $type == 'circle' ) ? 'social_linkedin_circle' : 'fa fa-linkedin';
				break;
			case 'tumblr':
				$icon = ( $type == 'circle' ) ? 'social_tumblr_circle' : 'fa fa-tumblr';
				break;
			case 'pinterest':
				$icon = ( $type == 'circle' ) ? 'social_pinterest_circle' : 'fa fa-pinterest';
				break;
			case 'vk':
				$icon = 'fa fa-vk';
				break;

<?php 
namespace ConallEdgeNamespace\Modules\Shortcodes\GalleryBlocksMasonryItem;

use ConallEdgeNamespace\Modules\Shortcodes\ShortcodeInterface;
/**
	* class GalleryBlocksMasonryItem
*/
class GalleryBlocksMasonryItem implements ShortcodeInterface {
	/**
	 * @var string
	 */
	private $base;
	function __construct() {
		$this->base = 'edgtf_gallery_blocks_masonry_item';
	...
	public function vcMap() {
		if(function_exists('vc_map')){
			vc_map( array(
				"name" => esc_html__('Edge Gallery Blocks Masonry Item', 'edgtf-core'),
				"base" => $this->base,
				"as_child" => array('only' => 'edgtf_gallery_blocks_masonry'),
				"category" => 'by EDGE',
				"icon" => "icon-wpb-gallery-blocks-masonry-item extended-custom-icon",
				"params" => array(
					array(
						'type' => 'dropdown',
						'class' => '',
					...
					array(
						'type'			=> 'textfield',
						'heading'		=> 'Custom Image Link',
						'param_name'	=> 'custom_image_link',
						'description'	=> 'If you set custom image link then lightbox functionality will be disabled',
					),
					array(
	                    'type'       => 'dropdown',
	                    'heading'    => 'Custom Image Link Target',
	                    ...
	                    'save_always' => true,
	                    'dependency' => array('element' => 'custom_image_link', 'not_empty' => true),
	                ),
					array(
						'type'			=> 'dropdown',
                    ...
                    array(
                        'type'			=> 'textfield',
                        'heading'		=> 'Custom Lightbox ID',
                        'param_name'	=> 'custom_lightbox_id',
                        'description'	=> 'If you want to merge images into one Lightbox, insert same ID',
                        'dependency' => array('element' => 'block_item_type', 'value' => array('edgtf-image-block'))
                    ),
	                array(
						'type' => 'colorpicker',
	...
	public function render($atts, $content = null) {
		...
		$default_atts = (array(
			...
			'image_size'	    			=> 'full',
			'custom_image_link'				=> '',
			'custom_image_link_target'		=> '_self',
			'enable_lightbox'   			=> '',
			'custom_lightbox_id'   			=> '',
			'background_color' 				=> '',
			'block_padding'					=> '',
        ...
        $params['lightbox_id'] = $params['custom_lightbox_id'] === '' ? rand(0,1000) : $params['custom_lightbox_id'];
		...
		$output = conall_edge_get_shortcode_module_template_part('templates/gallery-block-item-template', 'gallery-blocks-masonry', '', $params);

<?php
namespace ConallEdgeNamespace\Modules\Shortcodes\Separator;
...
class Separator implements ShortcodeInterface{
	...
	public function render($atts, $content = null) {
		...
		$html = conall_edge_get_shortcode_module_template_part('templates/separator-template', 'separator', '', $params);
	...
	private function getSeparatorStyle($params) {
		...
		if ($params['width'] !== '') {
			if(conall_edge_string_ends_with($params['width'], '%') || conall_edge_string_ends_with($params['width'], 'px')) {
		...
		if ($params['top_margin'] !== '') {
			if(conall_edge_string_ends_with($params['top_margin'], '%') || conall_edge_string_ends_with($params['top_margin'], 'px')) {
		...
		if ($params['bottom_margin'] !== '') {
			if(conall_edge_string_ends_with($params['bottom_margin'], '%') || conall_edge_string_ends_with($params['bottom_margin'], 'px')) {

...
if( ! class_exists('ConallEdgeClassSidebar') ){
	
	class ConallEdgeClassSidebar{
		...
		function __construct(){
			...
			$this->title = esc_html__('Custom Widget Area','conall');
		...
		function load_assets(){
			...
			wp_enqueue_script('conall-sidebar' , EDGE_ROOT . '/framework/admin/assets/js/edgtf-sidebar.js');
			wp_enqueue_style( 'conall-sidebar' , EDGE_ROOT . '/framework/admin/assets/css/edgtf-sidebar.css');
		}
		...
		function template_add_widget_field(){
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '".esc_html__('Enter Name of the new Widget Area', 'conall')."' name='edgt-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='".esc_html__('Add Widget Area', 'conall')."' />";
			echo "\n    ".$nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function delete_sidebar_area(){
			...
			if(!empty($_POST['name'])){
				...
				if(($key = array_search($name, $this->sidebars)) !== false){
					...
					update_option($this->stored, $this->sidebars);
					echo esc_html__( "sidebar-deleted", 'conall' );
				}
			}
		...
		function register_custom_sidebars(){
			...
			$args = apply_filters('conall_edge_custom_widget_args', $args);

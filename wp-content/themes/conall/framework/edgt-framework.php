...
if(!function_exists('conall_edge_enqueue_admin_scripts')) {
	...
	function conall_edge_enqueue_admin_scripts() {
		...
		wp_enqueue_media();
		wp_enqueue_script("conall-admin-dependence", get_template_directory_uri().'/framework/admin/assets/js/edgtf-ui/edgtf-dependence.js', array(), false, true);
        wp_enqueue_script("conall-admin-twitter-connect", get_template_directory_uri().'/framework/admin/assets/js/edgtf-twitter-connect.js', array(), false, true);
...
if(!function_exists('conall_edge_enqueue_meta_box_scripts')) {
	...
	function conall_edge_enqueue_meta_box_scripts() {
		...
		wp_enqueue_script('postbox');
		wp_enqueue_media();
		wp_enqueue_script('conall-admin-dependence');
...
if(!function_exists('conall_edge_enqueue_nav_menu_script')) {
	...
	function conall_edge_enqueue_nav_menu_script($hook) {
		if($hook == 'nav-menus.php') {
			wp_enqueue_script('conall-admin-nav-menu', get_template_directory_uri().'/framework/admin/assets/js/edgtf-nav-menu.js');
			wp_enqueue_style('conall-admin-nav-menu', get_template_directory_uri().'/framework/admin/assets/css/edgtf-nav-menu.css');
		}
	}
...
if(!function_exists('conall_edge_save_options')) {
	...
	function conall_edge_save_options() {
		...
		if(current_user_can('administrator')) {
	        ...
	        check_ajax_referer('conall_edge_ajax_save_nonce', 'conall_edge_ajax_save_nonce');
...
if(!function_exists('conall_edge_meta_box_add')) {
	...
	function conall_edge_meta_box_add() {
		...
		foreach ($conall_edge_framework->edgtMetaBoxes->metaBoxes as $key=>$box ) {
			...
			if(is_array($box->scope) && count($box->scope)) {
				foreach($box->scope as $screen) {
					conall_edge_create_meta_box_handler( $box, $key, $screen );
		...
		if ( conall_edge_is_gutenberg_installed() || class_exists( 'WP_Block_Type' ) ) {
			conall_edge_enqueue_meta_box_styles();
			conall_edge_enqueue_meta_box_scripts();
		} else {
...
if(!function_exists('conall_edge_generate_icon_pack_options')) {
    ...
    function conall_edge_generate_icon_pack_options() {
        ...
        $collections_object = conall_edge_icon_collections()->getIconCollection($icon_pack);
        ...
        echo conall_edge_get_module_part( $html );
    }

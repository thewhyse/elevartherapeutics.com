<?php
/*
   Class: ConallEdgeClassFramework
...
class ConallEdgeClassFramework {
    ...
    private function __construct() {
        $this->edgtOptions = ConallEdgeClassOptions::get_instance();
        $this->edgtMetaBoxes = ConallEdgeClassMetaBoxes::get_instance();
    }
	...
	public function setSkin(ConallEdgeClassSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class ConallEdgeClassSkinManager
 *
...
/**
 * Class ConallEdgeClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class ConallEdgeClassSkinAbstract {
    /**
     * @var string
     */
    protected $skinName;
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if($template !== '') {
            include(conall_edge_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
        }
    }
    ...
    public function setShortcodeJSParams() { ?>
		<script>
			window.edgtSCIcon = '<?php echo conall_edge_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
...
class ConallEdgeClassOptions {
    ...
    public function getOptionValue($key) {
        ...
        if(array_key_exists($key, $conall_edge_options)) {
            return $conall_edge_options[$key];
...
/*
   Class: ConallEdgeClassAdminPage
   A class that initializes Edge Admin Page
*/
class ConallEdgeClassAdminPage implements iConallEdgeInterfaceLayoutNode {
    ...
    function __construct($slug="", $title_label="", $icon = "") {
        $this->layout = array();
        $this->factory = new ConallEdgeClassFieldFactory();
        $this->slug = $slug;
    ...
    public function renderChild(iConallEdgeInterfaceRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: ConallEdgeClassMetaBoxes
...
/*
   Class: ConallEdgeClassMetaBox
   A class that initializes Edge Meta Box
*/
class ConallEdgeClassMetaBox implements iConallEdgeInterfaceLayoutNode {
	...
	public function setTitle($label) {
		...
		return $conall_edge_framework->getSkin()->getSkinLabel().' '.$label;
 	}
}

if ( ! function_exists( 'conall_edge_init_framework_variable' ) ) {

    function conall_edge_init_framework_variable() {

        global $conall_edge_framework;

        $conall_edge_framework = ConallEdgeClassFramework::get_instance();
        $edgtSkinManager = new ConallEdgeClassSkinManager();
        $conall_edge_framework->setSkin($edgtSkinManager->getSkin());
    }

    add_action('conall_edge_before_options_map', 'conall_edge_init_framework_variable' );
}
?>
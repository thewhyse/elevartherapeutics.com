...
if( ! empty($row_type) && $row_type == 'parallax'){
	...
	if(conall_edge_options()->getOptionValue('parallax_on_off') == 'off'){
...
if($video == 'show_video'){
	...
	if($video_image) {
		$video_output .= '<div class="edgtf-mobile-video-image" ' . conall_edge_get_inline_attr($video_mobile_style, 'style') . '></div>';
	}
	$video_output .= '<div ' . conall_edge_get_class_attribute($video_overlay_class) . conall_edge_get_inline_attr($video_overlay_style, 'style') . '></div>';
	$video_output .= '<div class="edgtf-video-wrap">';
	$video_output .= '<video class="edgtf-video" width="1920" height="800" '. conall_edge_get_inline_attr($video_attrs, 'poster')  .' controls="controls" preload="auto" loop autoplay muted>';
	...
	if($v_image) {
		$video_output .= '<img ' . conall_edge_get_inline_attr($v_image, 'src') . ' width="1920" height="800" title="' . esc_attr__( 'No video playback capabilities', 'conall' ) . '"  alt="'.esc_html__('video thumb','conall').'" />';
	}
	$video_output .='</object>';
	$video_output .='</video>';
	$video_output .='</div>';
...
echo conall_edge_get_module_part($output);